...
app.get("/resource-usage", async (req, res) => {
  ...
  const memoryInfo = {
    usage: ((os.totalmem() - os.freemem()) / 1024 / 1024).toFixed(2),
...
app.get("/fetch-image", async (req, res) => {
  ...
  try {
    ...
    const minimumFreeMemory = 200 * 1024 * 1024; // 200 MB in bytes

    // Check if URL is in cache
    if (cache[url]) {
      console.log("Serving from cache");
      res.setHeader("Content-Type", cache[url].contentType);
      return res.send(cache[url].data);
    }

    // Fetch and cache the resource if enough memory is available
    if (freeMemory >= minimumFreeMemory) {
      ...
      res.status(500).send("Not enough memory to cache the resource");
    }
  } catch (error) {
